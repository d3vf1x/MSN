...
static void hal_spi_init()
{
    ...
    // check if SPI bus is activated
    ret = spi_bus_initialize(LMIC_SPI, &buscfg, SPI_DMA_CH_AUTO); // 1
...
static void hal_time_init()
{
    ...
    int timer_idx = TIMER_0; // TIMER_1;
    timer_config_t config;
    config.alarm_en = 0;
    config.auto_reload = 0;
    config.counter_dir = TIMER_COUNT_UP;
    config.divider = 1600; // 1600
    config.intr_type = 0;
    config.counter_en = TIMER_PAUSE;
    config.clk_src = GPTIMER_CLK_SRC_DEFAULT; // GPTIMER_CLK_SRC_APB
    /*Configure timer*/
...
void set_sleep_time(uint64_t time){
    sleep_time = time;
} 
...
{
    uint64_t val;
    timer_get_counter_value(TIMER_GROUP_0, TIMER_0, &val); // TIMER_1
    return (u4_t)(val + offset);
}
...
void hal_sleep()
{
    ...
    if (sleep_time < os_getTime())
    {
        return;
    }
    uint64_t diff = (sleep_time  - os_getTime()) * 20;
    if(diff > 5000){
        ESP_LOGI(TAG, "Sleeping before %lu, %llu", os_getTime(),(diff - 1000)/1000);
        //ESP_ERROR_CHECK(esp_sleep_enable_gpio_wakeup());
        ESP_ERROR_CHECK(esp_sleep_enable_timer_wakeup(diff - 1000));
        ESP_ERROR_CHECK(esp_light_sleep_start());
        offset += (diff - 1000) / 20;
        ESP_LOGI(TAG, "Sleeping after %lu, offset %llu", os_getTime(),offset);
    }
