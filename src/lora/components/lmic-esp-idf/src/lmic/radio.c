...
static void txlora()
{
    ...
    u1_t cr = getCr(LMIC.rps);
     ESP_LOGI(TAG, "%lu: TXMODE, freq=%lu, len=%d, SF=%d, BW=%d, CR=4/%d, IH=%d",
...
static void rxlora(u1_t rxmode)
{
    ...
    else
    {
        ...
        u1_t bw = getBw(LMIC.rps);
        u1_t cr = getCr(LMIC.rps);
         ESP_LOGI(TAG, "%lu: %s, freq=%lu, SF=%d, BW=%d, CR=4/%d, IH=%d",
                    os_getTime(),
...
void radio_irq_handler(u1_t dio) 
{
    ...
    { // LORA modem
        u1_t flags = readReg(LORARegIrqFlags);
         ...
         ESP_LOGI(TAG,"%ld: irq: dio: 0x%x flags: 0x%x", now, dio, flags);
#endif
