...
static void updateTx(ostime_t txbeg)
{
    ...
    if (LMIC.globalDutyRate != 0)
     ...
     ESP_LOGI(LMIC_TAG, "%lu: Updating info for TX at %lu, airtime will be %lu. Setting available time for band %ld to %lu", os_getTime(), txbeg, airtime, freq & 0x3, band->avail);
    if (LMIC.globalDutyRate != 0)
         ESP_LOGI(LMIC_TAG, "%lu: Updating global duty avail to %lu", os_getTime(), LMIC.globalDutyAvail);
#endif
}
...
static ostime_t nextTx(ostime_t now)
{
    ...
    do
    {
        ...
        for (u1_t bi = 0; bi < 4; bi++)
        {
            ...
            {
#if LMIC_DEBUG_LEVEL > 1
                 ESP_LOGI(LMIC_TAG, "%lu: Considering band %d, which is available at %lu", os_getTime(), bi, LMIC.bands[bi].avail);
#endif
...
#if LMIC_DEBUG_LEVEL > 1
         ESP_LOGI(LMIC_TAG, "%lu: No channel found in band %d", os_getTime(), band);
#endif
        if ((bmap &= ~(1 << band)) == 0)
        {
...
#if LMIC_DEBUG_LEVEL > 1
    if (failed)
         ESP_LOGI(LMIC_TAG, "%lu: Join failed", os_getTime());
    else
         ESP_LOGI(LMIC_TAG, "%lu: Scheduling next join at %lu", os_getTime(), LMIC.txend);
#endif
...
static bit_t decodeFrame(void)
{
    ...
    norx:
         ...
         ESP_LOGI(LMIC_TAG, "%lu: Invalid downlink, window=%s", os_getTime(), window);
#endif
        LMIC.dataLen = 0;
        return 0;
    }
...
#if LMIC_DEBUG_LEVEL > 0
     ESP_LOGI(LMIC_TAG, "%lu: Received downlink, window=%s, port=%d, ack=%d", os_getTime(), window, port, ackup);
#endif
    return 1;
}
...
#if LMIC_DEBUG_LEVEL > 1
                 ESP_LOGI(LMIC_TAG, "%lu: Setup channel, idx=%d, freq=%lu", os_getTime(), chidx, (unsigned long)freq);
#endif
            }
        }
    }
...
static void buildDataFrame(void)
{
    ...
    {
        EV(devCond, INFO, (e_.reason = EV::devCond_t::RE_TX, e_.eui = MAIN::CDEV->getEui(), e_.info = LMIC.seqnoUp - 1, e_.info2 = ((LMIC.txCnt + 1) | (TABLE_GET_U1(DRADJUST, LMIC.txCnt + 1) << 8) | ((LMIC.datarate | DR_PAGE) << 16))));
    }
...
{
#if LMIC_DEBUG_LEVEL > 0
     ESP_LOGI(LMIC_TAG, "%lu: engineUpdate, opmode=0x%x", os_getTime(), LMIC.opmode);
#endif
    ...
    if ((LMIC.opmode & (OP_JOINING | OP_REJOIN | OP_TXDATA | OP_POLL)) != 0)
    {
...
#if LMIC_DEBUG_LEVEL > 1
        if (jacc)
             ESP_LOGI(LMIC_TAG, "%lu: Uplink join pending", os_getTime());
        else
             ESP_LOGI(LMIC_TAG, "%lu: Uplink data pending", os_getTime());
#endif
        ...
        if ((LMIC.opmode & OP_NEXTCHNL) != 0)
        {
            ...
            LMIC.opmode &= ~OP_NEXTCHNL;
#if LMIC_DEBUG_LEVEL > 1
             ESP_LOGI(LMIC_TAG, "%lu: Airtime available at %lu (channel duty limit)", os_getTime(), txbeg);
#endif
        }
        else
        {
            txbeg = LMIC.txend;
#if LMIC_DEBUG_LEVEL > 1
             ESP_LOGI(LMIC_TAG, "%lu: Airtime available at %lu (previously determined)", os_getTime(), txbeg);
#endif
        }
...
#if LMIC_DEBUG_LEVEL > 1
             ESP_LOGI(LMIC_TAG, "%lu: Airtime available at %lu (global duty limit)", os_getTime(), txbeg);
#endif
        }
#if !defined(DISABLE_BEACONS)
...
#if LMIC_DEBUG_LEVEL > 1
             ESP_LOGI(LMIC_TAG, "%lu: Awaiting beacon before uplink", os_getTime());
#endif
       ...
       // Earliest possible time vs overhead to setup radio
        ...
        {
#if LMIC_DEBUG_LEVEL > 1
             ESP_LOGI(LMIC_TAG, "%lu: Ready for uplink", os_getTime());
#endif
...
#if LMIC_DEBUG_LEVEL > 1
         ESP_LOGI(LMIC_TAG, "%lu: Uplink delayed until %lu", os_getTime(), txbeg);
#endif
        // Cannot yet TX
        if ((LMIC.opmode & OP_TRACK) == 0)
